...
5. **当地美食推荐**：按城市分组，代表菜 + 推荐店（人均 · 营业时间 · 高德导航按钮）
6. **出行提醒**（实时数据）：恶劣天气预警、道路/景区临时关闭、限流公告
7. 门票与花费表（票价 · 预约说明 · 人均合计 · 价格时效声明）
8. 穿着建议（按场景 + 最低夜温分层）
9. 注意事项（行车安全、高反、车况检查、预约、异地救援、离线地图）
10. 页脚：“天气为高德预报（更新时间）· 里程为高德驾车规划实测”
...
8. **当地美食推荐**：按城市分组，代表菜 + 推荐店（评分来源 · 价位 · 营业时间与休息日 · 是否预约 · Google 地图导航按钮）
9. 门票与预算：住宿/城际交通/市内交通/门票/餐饮分项，外币 + 人民币（**汇率为实时数据**，显示更新日期），城市税单列
10. 穿着建议（教堂遮肩膝、石板路步行鞋、雨具、山区保暖）
11. 实用信息：插头（多数 C/F，英国 G，瑞士 J，意大利 L）、时差、小费、退税、eSIM、支付习惯
12. 安全与应急：扒窃高发区与手法、欧盟急救 112、外交部全球领保热线 +86-10-12308、护照丢失处置
13. 页脚：数据来源与查询日期
...
- **地图二选一**：
  ...
  - `food`：当地美食推荐，结构 `[{city, items:[{name, tags[], desc, shops:[{name, note, map_url}]}]}]`，加 `food_note` 写来源与查询日期；店铺导航按钮只接受 https 链接。
